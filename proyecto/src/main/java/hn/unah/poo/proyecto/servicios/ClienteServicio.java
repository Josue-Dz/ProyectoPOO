...
public class ClienteServicio {
    /**
     * Variable que obiente la informacion del Cliente en JPARepository
     */
    @Autowired
    ...
    /**
     * Variable que obiente la informacion de la Tabla Amortizacion en JPARepository
     */
    @Autowired
    private TablaAmortizacionServicio tablaAmortizacionServicio;
    /**
     * Variable que obiente la informacion del Prestamo del cliente en JPARepository
     */
    @Autowired
     ...
     * 
     * @param nvoClienteDTO
     * @return Un mensaje indicando si el cliente fue creado exitosamente, o una
     ...
     * El objeto tiene la informacion del cliente y tambien puede llevar sus
     * direcciones o prestamos
     * 
     * @param clienteDTO
     * @return Retorna un mensaje que indica el resultado de la operación
     */
    public String crearClientes(@Valid ClienteDTO clienteDTO) {
        try {
            // Se mapea el cliente recibido del usuario y se guarda en la variable cliente
            Cliente cliente = SingletonModelMapper.getModelMapperInstance().map(clienteDTO, Cliente.class);

            // Verificar si el cliente ya existe
            Optional<Cliente> clienteExistente = clienteRepositorio.findById(cliente.getDni());
            if (clienteExistente.isPresent()) {
                return "Error: El cliente con DNI: " + cliente.getDni() + " ya existe en la base de datos.";
            }

            // Verificar que el cliente no tenga más de dos direcciones
            List<Direcciones> direcciones = cliente.getDirecciones();
            if (direcciones != null && direcciones.size() > 2) {
                return "Error: El cliente solo puede tener un máximo de dos direcciones.";
            }
            ...
            if (prestamos != null) {
                // Guardar los préstamos antes de guardar el cliente
                for (Prestamos prestamo : prestamos) {
                    String tipoPrestamo = this.prestamosServicio.cambiarAStringTipoPrestamo(prestamo.getTipoPrestamo());
                    double tasaInteres = this.prestamosServicio.obtenerTasaInteres(tipoPrestamo);
                    prestamo.setTasaInteres(tasaInteres);
                    prestamo.setCuota(prestamosServicio.obtenerCuotas(prestamo));

                    // Inicializamos la lista de clientes si está vacía
                    if (prestamo.getClientes() == null) {
                        prestamo.setClientes(new HashSet<>());
                    }

                    // Agregamos el cliente al conjunto de clientes del préstamo
            ...
            // Guardar el cliente después de haber guardado los préstamos
            Cliente clienteGuardado = clienteRepositorio.save(cliente);

            // Crear la tabla de amortización para los préstamos guardados
            Set<Prestamos> prestamosGuardados = clienteGuardado.getPrestamos();
            if (prestamosGuardados != null) {
                for (Prestamos prestamo : prestamosGuardados) {
                    tablaAmortizacionServicio.crearTablaAmortizacion(prestamo);
                }
            }
    ...
    public String agregarDireccion(String dni, DireccionesDTO direccionDTO) {
        try {
            ...
            if (this.clienteRepositorio.existsById(dni)
                    && !(this.clienteRepositorio.findById(dni).get().getDirecciones().size() < 2)) {
     ...
     * - Mapea las direcciones asociadas al cliente en una lista de
     * `DireccionesDTO`.
     ...
     * 
     * @param dni
     * @return Un objeto `ClienteDTO` con toda la información del cliente, o `null`
        ...
        try {
            ...
            if (cliente.getPrestamos() != null) {
                for (Prestamos prestamo : cliente.getPrestamos()) {
                    PrestamosDTO prestamoDTO = SingletonModelMapper.getModelMapperInstance().map(prestamo,
                            PrestamosDTO.class);
                    prestamoDTO.setTablaAmortizacionDTO(new ArrayList<>());

                    for (TablaAmortizacion cuota : prestamo.getTablaAmortizacion()) {
                        TablaAmortizacionDTO cuotaDTO = SingletonModelMapper.getModelMapperInstance().map(cuota,
                                TablaAmortizacionDTO.class);
     ...
     * - Mapea los préstamos asociados al cliente en un conjunto de `PrestamosDTO`.
     * 
     * @return Una lista de objetos `ClienteDTO`, donde cada elemento contiene la
    ...
    public List<ClienteDTO> obtenerTodos() {
        ...
        for (Cliente cliente : listaClientes) {
            ...
            if (cliente.getPrestamos() != null) {
                for (Prestamos prestamo : cliente.getPrestamos()) {
                    PrestamosDTO prestamosDTO = SingletonModelMapper.getModelMapperInstance().map(prestamo,
                            PrestamosDTO.class);
    ...
    /**
     * Obtiene una lista de todos los clientes con su respectiva direccion y
     * prestamos
     *
     * @return Una lista de ClienteDTO
     */
    public List<ClienteDTO> obtenerClientes() {
        try {
            // Variable que obtiene una lista de todos los clientes del sistema
            List<Cliente> clientes = this.clienteRepositorio.findAll();

            // Se mapea la lista de clientes y se agrega a una lista de ClienteDTO
            List<ClienteDTO> clientesDTO = clientes.stream()
                    .map(cliente -> {

                        // Se mapea cada cliente y se guarda en clienteDTO
                        ClienteDTO clienteDTO = SingletonModelMapper.getModelMapperInstance().map(cliente,
                                ClienteDTO.class);

                        // Si el cliente tiene ingresada alguna direccion que se guarde en el clienteDTO
                        if (cliente.getDirecciones() != null && !cliente.getDirecciones().isEmpty()) {
                            // Se Mapea las direcciones obtenidas del cliente y se guarda en la variable
                            // clienteDTO
                            List<DireccionesDTO> direccionDTO = cliente.getDirecciones().stream()
                                    .map(direccion -> SingletonModelMapper.getModelMapperInstance().map(direccion,
                                            DireccionesDTO.class))
                                    .collect(Collectors.toList());
                            // Se guarda o establece la direccion que se encontró en clienteDTO
                            clienteDTO.setDireccionesDTO(direccionDTO);
                        }

                        // Se mapean los prestamos del cliente si existen.
                        if (cliente.getPrestamos() != null && !cliente.getPrestamos().isEmpty()) {
                            // Se mapea los prestamos del cliente y se guardan en prestamosDTO
                            Set<PrestamosDTO> prestamosDTO = cliente.getPrestamos().stream()
                                    .map(prestamo -> {

                                        // Se mapea los prestamos de uno a uno y se guarda en prestamoDTO
                                        PrestamosDTO prestamoDTO = SingletonModelMapper.getModelMapperInstance()
                                                .map(prestamo, PrestamosDTO.class);

                                        // Dentro del prestamo del cliente se revisa si ya existe la tabla amortizacion
                                        if (prestamo.getTablaAmortizacion() != null
                                                && !prestamo.getTablaAmortizacion().isEmpty()) {
                                            // Se mapea la tabla Amortizacion y se guarda en una tabla amortizacionDTO
                                            List<TablaAmortizacionDTO> tablaAmortizacionesDTO = prestamo
                                                    .getTablaAmortizacion().stream()
                                                    .map(tablaAmortizacion -> SingletonModelMapper
                                                            .getModelMapperInstance().map(tablaAmortizacion,
                                                                    TablaAmortizacionDTO.class))
                                                    .collect(Collectors.toList());

                                            // Se guarda o establece las tablas de amortización encontradas en el
                                            // prestamo del cliente hacia el prestamoDTO
                                            prestamoDTO.setTablaAmortizacionDTO(tablaAmortizacionesDTO);
                                        }
                                        // Se retorna una variable de tipo PrestamosDTO con los prestamos del clienteDTO
                                        return prestamoDTO;
                                    })
                        ...
                        // Se retorna una variable de tipo ClienteDTO con la informacion obtenida del
                        // sistema
                        return clienteDTO;
                    })
                    .collect(Collectors.toList());
            // Retorna una lista de ClientesDTO con toda la informacion de los clientes.
            return clientesDTO;
        } catch (Exception e) {
            // En caso de erro se envia una lista vacia para que no muestre NULL
            return new ArrayList<>();
        }
    }

    /**
     * Elimina un cliente de la base de datos si cumple con las condiciones
     * necesarias.
     * 
     * Este método realiza las siguientes operaciones:
     * - Verifica si el cliente existe en la base de datos.
     ...
     * 
     * @param dni
     * @return Un mensaje indicando si la eliminación fue exitosa o enviando una
     *         excepcion.
     */
        ...
        try {
            ...
            for (Prestamos prestamo : cliente.getPrestamos()) {
                if (prestamo.getEstado() == 'P') {
                    return "El cliente con DNI: " + dni
                            + " no puede ser eliminado, posee prestamo(s) con saldo pendiente!";
                }
            }
     ...
     * - Actualiza los campos modificables del cliente, como correo, sueldo y
     * teléfono.
     * - Actualiza las direcciones asociadas al cliente, reemplazando las existentes
     * con las nuevas proporcionadas.
     * - Almacena los cambios en la base de datos.
     * 
     * @param dni
     * @param clienteDTO
     * @return Un mensaje indicando si la actualización fue exitosa o enviando una
     *         excepcion.
     */
            ...
            if ((clienteDTO.getDireccionesDTO() != null)) {
                ...
                if (clienteDTO.getDireccionesDTO().size() <= 2) {
                    ...
                    for (DireccionesDTO direccionDTO : clienteDTO.getDireccionesDTO()) {
                        Direcciones direccion = SingletonModelMapper.getModelMapperInstance().map(direccionDTO,
                                Direcciones.class);
